export default function HowItWorks() {
  ...
  return (
    <section id="how-it-works" className="py-24 px-6 bg-[#2B1A2E]">
      <div className="max-w-7xl mx-auto">
        {/* How Rose Works */}
        <div className="text-center mb-16">
          <div className="text-sm tracking-wider text-[#C9A14A] mb-4">HOW ROSE WORKS</div>
          <h2 className="text-5xl tracking-tight mb-4 text-[#F5EAE6]">Three steps. Every time.</h2>
          <p className="text-lg text-[#D9B8C0]">Rose doesn't just listen. She digs in.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
          {steps.map((step) => (
            <div key={step.number} className="group">
              ...
              <h3 className="text-2xl mb-3 tracking-tight text-[#F5EAE6]">{step.title}</h3>
              <p className="text-[#D9B8C0] leading-relaxed">{step.description}</p>
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-col items-center">
          <div className="w-[280px] h-px bg-[#8B5A6B]/40 mb-6" />
          <p className="text-[14px] text-[#D9B8C0] text-center mb-2">
            Grounded in research on feedback, leadership, and workplace equity.
          </p>
          <a
            href="/research"
            className="text-[14px] text-[#C9A14A] underline hover:text-[#B89140] transition-colors"
          >
